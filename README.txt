...
An executable called "quadric" will be produced.
...
• -lifemode plays the game in life mode
• -setlife x set the lives in the game to be x. If x is greater than 3, the number of lives is 3, if x is less than 0, the number of lives is 1 instead; this command is not effective when the game was not played in life-mode

Extra Feature:

• Skip block -Once a block has appeared, you can either decide use that block or skip that block. If you want to skip it, just type 'skip' in the command line.
• Life-mode -The game can be played in Life-mode which gives the player maximum 3 lives per game. 
