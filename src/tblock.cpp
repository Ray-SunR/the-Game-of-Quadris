...
TBlock::TBlock(Grid* gd, int level): AbstractBlock(4, level)
{
    shape = 'T';
    refcoord.x = 3;
    refcoord.y = 0;
...
void TBlock::clockwiseRotate(Grid* gd)
{
...
void TBlock::counterClockwiseRotate(Grid* gd)
{
...
void TBlock::calculatePotentialPos(coordinate& fakeRef, vector<coordinate>& potentialVcoor)
{
	//there are four shapes for block T
    if (currentType == 0)
    {
        //first shape
...
coordinate TBlock::getRightCorner()
{
    coordinate temp;
    temp.x = refcoord.x;
    temp.y = refcoord.y;
